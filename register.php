...
<?php
  ...
  if(isset($_POST['email'])) {
	...
	if($emailMatch>0) {
	echo'<script type="text/javascript">alert("User with that email already exist."); window.location.href = "http://localhost/11/register.php";</script>';
	exit();
	}		else if(empty($name) || empty($email) || empty($password) || empty($password_again)) {
	echo'<script type="text/javascript">alert("Fields can\'t be empty."); window.location.href = "http://localhost/11/register.php";</script>';
	exit();
	}							
													
		$sql=mysql_query("INSERT INTO user (name, email, password, password_again) VALUES ('$name', '$email', '$password', '$password_again')")  or die (mysql_error());
		$query = mysql_query($sql);	
		if(!$emailMatch>0) {
	echo'<script type="text/javascript">alert("Registration Successful. You may login now."); window.location.href = "http://localhost/11/login.php";</script>';
	exit();
	}															
  }
...
<div class="wrapper">
    ...
    <!--Start of Container-->
		...
		<div class="login-form">
			<h2>Register</h2>
			<form action="register.php" controller="users" id="UserSignupForm" method="post" accept-charset="utf-8"><div style="display:none;"><input type="hidden" name="_method" value="POST"/></div><input name="name" placeholder="Name" class="usernames" maxlength="255" type="text" id="UserName"/><input name="email" placeholder="Email" class="email" maxlength="250" type="text" id="UserEmail"/><input name="password" placeholder="Password" class="password" type="password" id="UserPassword"/><input name="password_again" placeholder="Re-enter password" class="password" type="password" id="UserPsword"/>			<input name="" type="submit" name="register" value="Register" class="register-wrap">
			</form>
		</div> 
		</div>
	</div>
